Java-Reactive-API: Reactive Extensions for the JVM
...
Java-Reactive-API is a Java VM implementation of [Reactive Extensions](http://reactivex.io): a library for composing asynchronous and event-based programs by using observable sequences.
...
- fixed API mistakes and many limits ofJava-Reactive-API2
- intended to be a replacement forJava-Reactive-API2 with relatively few binary incompatible changes
...
Learn more aboutJava-Reactive-APIin general on the <a href="https://github.com/ReactiveX/RxJava/wiki">Wiki Home</a>.
...
The first step is to includeJava-Reactive-API3 into your project, for example, as a Gradle compile dependency:
...
Note thatJava-Reactive-API3 components now live under `io.reactivex.rxjava3` and the base classes and interfaces live under `io.reactivex.rxjava3.core`.
...
The dataflows inJava-Reactive-APIconsist of a source, zero or more intermediate steps followed by a data consumer or combinator step (where the step is responsible to consume the dataflow by some means):
...
One of the common use cases forJava-Reactive-APIis to run some computation, network request on a background thread and show the results (or error) on the UI thread:
...
RxJava operators don't work with `Thread`s or `ExecutorService`s directly but with so-called `Scheduler`s that abstract away sources of concurrency behind a uniform API.Java-Reactive-API3 features several standard schedulers accessible via `Schedulers` utility class. 
...
Practically, parallelism inJava-Reactive-APImeans running independent flows and merging their results back into a single flow. The operator `flatMap` does this by first mapping each number from 1 to 10 into its own individual `Flowable`, runs them and merges the computed squares.
...
For R8, theJava-Reactive-APIjar includes the `META-INF/proguard/rxjava3.pro` with the same no-warning clause and should apply automatically.
...
## LICENSE
    ...
    Copyright (c) 2016-present,Java-Reactive-APIContributors.
